...
async def handle_song_callback(client: Bot, callback: CallbackQuery):
    ...
    try:
        ...
        logger.error(e)
        traceback.print_exc()
        return await msg.edit("ᴄᴏɴɴᴇᴄᴛɪᴏɴ ʀᴇғᴜsᴇᴅ ʙʏ ᴀᴘɪ, ᴘʟᴇᴀsᴇ ᴛʀʏ ʟᴀᴛᴇʀ ᴏɴ!")
...
async def lyrics(client: Bot, callback: CallbackQuery):
    ...
    if not lyrics:
        await callback.answer("**ᴛʜᴇ ʀᴇϙᴜᴇsᴛᴇᴅ sᴏɴɢ's ʟʏʀɪᴄs ᴄᴏᴜʟᴅ ɴᴏᴛ ʙᴇ ғᴏᴜɴᴅ**", show_alert=True)
        return
    ...
    if len(lyrics) <= 4096:
        ...
        file_location = f"{response.get("snippet")} lyrics.txt"
