...
async def search(client: Bot, message: Message|CallbackQuery):
    ...
    try:
        ...
        logger.error(e)
        traceback.print_exc()
        return await send_msg.edit("ᴄᴏɴɴᴇᴄᴛɪᴏɴ ʀᴇғᴜsᴇᴅ ʙʏ ᴀᴘɪ, ᴘʟᴇᴀsᴇ ᴛʀʏ ᴀɢᴀɪɴ!")
    ...
    if search_type == "all" or search_type == "topquery":
        ...
        navigation_buttons = []
        if page_no > 1:
            navigation_buttons.append(InlineKeyboardButton("⇇", callback_data=f"search#{search_type}#{page_no-1}"))
