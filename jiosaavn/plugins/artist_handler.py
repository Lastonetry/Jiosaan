...
async def artist(client: Bot, callback: CallbackQuery):
    ...
    try:
        ...
        logger.error(e)
        traceback.print_exc()
        return await msg.edit("ᴄᴏɴɴᴇᴄᴛɪᴏɴ ʀᴇғᴜsᴇᴅ ʙʏ sᴘᴏᴛɪғʏ ᴀᴘɪ, ᴘʟᴇᴀsᴇ ᴛʀʏ ᴀɢᴀɪɴ !!")
    ...
    navigation_buttons = []
    if page_no > 1:
        navigation_buttons.append(InlineKeyboardButton("⇇ ʙᴀᴄᴋ", callback_data=f"artist#{artist_id}#{page_no-1}"))
    if total_results > 10 * page_no:
        navigation_buttons.append(InlineKeyboardButton("ɴᴇxᴛ ⇉", callback_data=f"artist#{artist_id}#{page_no+1}"))
